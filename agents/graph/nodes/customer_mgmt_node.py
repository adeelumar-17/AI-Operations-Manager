"""Customer lookup and communication history"""
import logging
...
def _get_tools_for_customer() -> list:
    relevant = {"search_customer", "get_customer_details", "get_customer_history", "get_communication_history", "log_communication"}
...
def customer_mgmt_node(state: AgentState) -> dict:
    results = []
    try:
        tools = {t.name: t for t in _get_tools_for_customer()}
        llm = get_llm().bind_tools(list(tools.values()))
        prompt = state['user_input'] + '\nCurrent UTC time: ' + datetime.now(timezone.utc).isoformat() + '\nUse search_customer to resolve names, then inspect profiles or history. Customer record creation and editing are not available; explain the limitation when requested.'
        ...
        return {'action_results': results, 'action_plan': [{'action': 'customer_management', 'entities': state.get('entities', {})}]}
    except Exception as exc:
        logger.exception('Customer lookup and communication history failed')
